...
import torch.nn.functional as F  # Import functional module for Huber loss
import warnings
...
class OSRSMarketDataset(Dataset):
    def __init__(self):
        ...
        def create_sequences(item_data_list):
            ...
            for i in range(n - 488):
                ...
                sequence = np.column_stack((
                    (features[:, :2] - price_mean) / price_std,  # Scale price features
                    (features[:, 2:] - vol_mean) / vol_std       # Scale volume features
                ))
        ...
        for typeid, data_list in item_data.items():
            ...
            average_volume = np.mean(seq[:, 2] + seq[:, 4])  # Avg(high volume + low volume)
...
for sample in val_dataset:
    ...
    loss = F.huber_loss(forecast_tensor, test_data_tensor, delta=1.35)
    huber_loss_list.append(loss.item())
    print("Mean:", np.mean(huber_loss_list))

# Calculate and print the average HuberLoss over the entire dataset.
avg_huber_loss = np.mean(huber_loss_list)
print("Average HuberLoss over dataset:", avg_huber_loss)
